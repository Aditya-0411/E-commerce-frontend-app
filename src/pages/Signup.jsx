...
const Signup = () => {
    const navigate = useNavigate();
    const [formData, setFormData] = useState({
        name: '', phone_number: '', email: '', gender: '', date_of_birth: '', password: '', password2: '',
        referral_code: ''
    });
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    ...
    const handleSubmit = async (e) => {
        ...
        // Basic frontend validation before proceeding
        if (activeTab === 0) {
            const { name, phone_number, email, date_of_birth, gender } = formData;
            if (!name || !phone_number || !email || !date_of_birth || !gender) {
                toast.warn("Please fill all required fields in Personal Info.");
                return;
            }
            setActiveTab(1);
            return;
                ...
                >
                    ...
                    <Box textAlign="center" mb={2}>
                        <Avatar sx={{ bgcolor: 'secondary.main', width: 56, height: 56, m: '0 auto', mb: 2 }}>
                            <PersonOutlineIcon sx={{ fontSize: 32, color: 'background.paper' }} />
                        </Avatar>
                        <Typography variant="h4" component="h1" fontWeight={700} sx={{ color: 'secondary.main' }}>
                            Create Your Zirvana Account
                        </Typography>
                    </Box>

                    {error && (
                        <Alert severity="error" variant="filled" sx={{ mb: 2, whiteSpace: 'pre-wrap' }}>{error}</Alert>
                    )}
                    ...
                    <Box component="form" onSubmit={handleSubmit} noValidate>
                        ...
                        <Tabs
                            ...
                            sx={{ mb: 1, '& .MuiTab-root': { color: 'text.secondary' } }}
                        >
                            <Tab label="1. Personal Info" />
                            <Tab label="2. Security" />
                        </Tabs>
                        ...
                        <TabPanel value={activeTab} index={0}>
                            <Grid container spacing={2}>
                                <Grid item xs={12}><TextField required fullWidth size="small" label="Full Name" name="name" value={formData.name} onChange={handleChange} /></Grid>
                                <Grid item xs={12} sm={6}><TextField required fullWidth size="small" label="Phone Number" name="phone_number" type="tel" value={formData.phone_number} onChange={handleChange} InputProps={{ startAdornment: (<InputAdornment position="start"><PhoneIphoneIcon sx={{ color: 'text.secondary' }} /></InputAdornment>) }} /></Grid>
                                <Grid item xs={12} sm={6}><TextField required fullWidth size="small" label="Email" name="email" type="email" value={formData.email} onChange={handleChange} InputProps={{ startAdornment: (<InputAdornment position="start"><EmailIcon sx={{ color: 'text.secondary' }} /></InputAdornment>) }} /></Grid>
                                <Grid item xs={12} sm={6}><TextField required fullWidth size="small" label="Date of Birth" name="date_of_birth" type="date" InputLabelProps={{ shrink: true }} value={formData.date_of_birth} onChange={handleChange} /></Grid>
                                <Grid item xs={12} sm={6}>
                                    <TextField required select label="Gender" name="gender" value={formData.gender} onChange={handleChange} fullWidth size="small" InputProps={{ startAdornment: (<InputAdornment position="start"><WcIcon sx={{ color: 'text.secondary' }} /></InputAdornment>) }}>
                                        <MenuItem value="M">Male</MenuItem>
                                        <MenuItem value="F">Female</MenuItem>
                                        <MenuItem value="O">Other</MenuItem>
                                    </TextField>
                                </Grid>
                                <Grid item xs={12}><TextField fullWidth size="small" label="Referral Code (Optional)" name="referral_code" value={formData.referral_code} onChange={handleChange} InputProps={{ startAdornment: (<InputAdornment position="start"><ShareIcon sx={{ color: 'text.secondary' }} /></InputAdornment>) }} /></Grid>
                            </Grid>
                            <Button variant="contained" color="primary" size="large" fullWidth onClick={() => setActiveTab(1)} sx={{ mt: 3, py: 1.5 }}>Next: Set Password</Button>
                        </TabPanel>

                        {/* Tab Content 2: Security */}
                        <TabPanel value={activeTab} index={1}>
                            <Grid container spacing={2}>
                                <Grid item xs={12}><TextField required fullWidth size="small" label="Password" name="password" type={showPassword ? 'text' : 'password'} value={formData.password} onChange={handleChange} InputProps={{ startAdornment: ( <InputAdornment position="start"><LockOutlinedIcon sx={{ color: 'text.secondary' }} /></InputAdornment> ), endAdornment: ( <IconButton onClick={togglePasswordVisibility} edge="end"><small>{showPassword ? <VisibilityOff /> : <Visibility />}</small></IconButton> ) }} /></Grid>
                                <Grid item xs={12}><TextField required fullWidth size="small" label="Confirm Password" name="password2" type={showPassword ? 'text' : 'password'} value={formData.password2} onChange={handleChange} /></Grid>
                            </Grid>
                            <Button type="submit" variant="contained" color="primary" size="large" fullWidth disabled={loading} sx={{ mt: 3, py: 1.5 }}>
                                {loading ? <CircularProgress size={26} color="inherit" /> : 'Create Account'}
                            </Button>
                            <Button onClick={() => setActiveTab(0)} variant="text" fullWidth sx={{ mt: 1 }} disabled={loading}>Back to Personal Info</Button>
                        </TabPanel>
                    </Box>
                    ...
                    <Box sx={{ mt: 3, textAlign: 'center' }}>
                        <Typography variant="body2" color="text.secondary">
                            Already have an account?
                            <Link to="/login" style={{ textDecoration: 'none', color: '#00bfa5', fontWeight: 600, marginLeft: 4 }}>
                                Log In
                            </Link>
                        </Typography>
                    </Box>
