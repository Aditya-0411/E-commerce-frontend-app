          ...
          <Accordion>
            ...
            <AccordionDetails>
                ...
                {order.items.map((item) => (
                  <ListItem key={item.id}>
                    <ListItemText 
                      primary={
                        <>
                          {item.title_snapshot} x {item.qty}
                          {item.is_prebook && (
                            <Typography variant="caption" color="primary" sx={{ display: 'block' }}>
                              Prebooked for ₹{item.prebook_amount}
                            </Typography>
                          )}
                        </>
                      } 
                      secondary={
                        item.is_prebook 
                          ? "Prebooking Pending Full Payment"
                          : `Price at purchase: ₹${item.price_snapshot}`
                      } 
                    />
                  </ListItem>
                ))}
              ...
              <Box sx={{ p: 1 }}>
                ...
                {order.is_prebook && (
                  <Typography variant="body2" color="primary">Prebook Paid: ₹{order.prebook_paid}</Typography>
                )}
